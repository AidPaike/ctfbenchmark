...
Writes progress to the given terminal row using ANSI escapes.
"""
...
def main() -> None:
    ...
    while True:
        try:
            req = urllib.request.Request(
                f"{api}/api/challenges/prefetch/progress",
                headers={"Authorization": f"Bearer {token}"},
            )
            opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))
        ...
        _write(row, f"  \033[1mPrefetch\033[0m   \033[36m⟳\033[0m [{bar}] {pct:3d}%  {current}/{total}  \033[2m{cid}\033[0m   ")

        time.sleep(1)


def _write(row: int, text: str) -> None:
    """Write text to a specific terminal row, then restore cursor."""
