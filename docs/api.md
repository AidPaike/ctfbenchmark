...
本文档描述当前实现的 Droplet HTTP API。当前版本采用单机黑盒评测模型：没有多用户 session，也没有同题多副本 attempt；平台直接管理每道题目的服务生命周期。全局重置内部使用 reset epoch 隐藏旧进度和旧活动链，这是单用户进度代次，不是对外用户 session。
...
Agent 可见的题目对象不会返回源码、Dockerfile、Compose、`.env`、flag 或 hint 内容。题目对象只返回 `has_hint`；hint 内容必须通过 hint 接口获取，以便记录扣分。
...
- `not_started`：题目服务未运行
- `starting`：后台 Docker Compose 启动中
- `running`：题目服务已启动，Agent 可以通过端口访问
- `stopping`：后台 Docker Compose 停止中
...
清理前端活动链视图。该接口不会物理删除审计日志，只会在当前 reset epoch 中把对应事件标记为 archived，后续列表接口不再展示。
...
异步启动单道题目的真实 Docker Compose 服务，返回 `202 Accepted` 和当前 `Challenge`。刚返回时通常是 `starting`，客户端应轮询 `GET /api/challenges/{challenge_id}` 直到 `running` 或 `error`。如果该题已经在运行，接口直接返回当前运行状态；需要重启请调用 `reset`。

### `POST /api/challenges/{challenge_id}/stop`

异步停止单道题目服务并清理 `data/work/challenges/{challenge_id}`，返回 `202 Accepted`。客户端应轮询到 `not_started` 或 `error`。

### `POST /api/challenges/{challenge_id}/reset`

异步停止并重新启动单道题目服务，返回 `202 Accepted`。当前 reset epoch 内该题的提示状态、提交次数和分数会清零；历史提交仍保留在数据库中，但当前视图不再展示。
...
获取提示。首次查看该题提示扣 0.1 分，重复查看不再重复扣分。
...
默认后端启动时会发起题目预启动，但会受 `DROPLET_MAX_CONCURRENT_ENVIRONMENTS` 限制；默认最多同时运行 2 个题目环境：
...
该命令会启动题目并检查每道被选中的题是否 `running` 且有端口；如果有任何题目失败或因并发上限未启动，命令返回非 0，输出中会列出失败题目的 `id`、`status`、`target_url`、`ports` 和 `error_message`。这一步通过后，再让 Agent 获取 `/api/v1/challenges` 开始答题。

如果需要一次性预启动全部 5 道 demo 题，需要在启动后端前调大并发上限：

```bash
DROPLET_MAX_CONCURRENT_ENVIRONMENTS=5 ./scripts/dev/dev-backend.sh
```
...
```bash
DROPLET_PRESTART_IDS=xben-001-24,xben-002-24 ./scripts/dev/dev-backend.sh
```

单道题 Docker Compose 启动超时默认是 300 秒，避免首次镜像构建过慢时被误杀。需要更长时间时可以继续调大：

```bash
DROPLET_COMPOSE_TIMEOUT_SECONDS=300 ./scripts/dev/dev-backend.sh
```

默认启动命令是 `docker compose up -d`。如果镜像已经存在，会直接启动；如果镜像不存在，Compose 会按需构建。需要强制 rebuild 时，在启动后端前设置：

```bash
DROPLET_FORCE_REBUILD=1 ./scripts/dev/dev-backend.sh
```
...
- 事件日志：SQLite 表 `events`，位于 `data/droplet.db`，供前端活动链和 API 查询使用。
- 旧事件迁移：`logs/droplet-events.jsonl` 只作为历史 JSONL 迁移来源，新事件不再写入 JSONL。
